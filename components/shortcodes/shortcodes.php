...
class EasyAzon_Components_Shortcodes {
	...
	public static function get_url($url, $shortcode_atts) {
		if(isset($shortcode_atts['identifier']) && isset($shortcode_atts['locale'])) {
		...
		} else if(isset($shortcode_atts['tag']) && !empty($shortcode_atts['tag'])) {
			$url = add_query_arg('tag', $shortcode_atts['tag'], $url);
		}
